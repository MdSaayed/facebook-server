...
exports.getProfile = async (req, res) => {
  try {
    ...
    if (user.friends.includes(profile._id) && profile.friends.includes) {
    ...
    const posts = await Post.find({ user: profile._id }).populate("user").populate('comments.commentBy', 'first_name last_name picture username commentAt').sort({ createdAt: -1 });
